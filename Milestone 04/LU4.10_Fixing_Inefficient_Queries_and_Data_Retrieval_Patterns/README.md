...
1. Open your `.env` file and paste your Neon database URL:
   ...
   > If Prisma reports `Can't reach database server`, make sure your Neon project is active and the connection string matches the current Neon dashboard. Free Neon clusters can pause when idle, so verify the database is running before starting the server.
